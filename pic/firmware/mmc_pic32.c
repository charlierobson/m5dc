...
#define CS_LOW()  SD_SELECT()	/* MMC CS = L */
#define CS_HIGH() SD_DESELECT()	/* MMC CS = H */

#define INS	(SD_CD==0)	/* Card detected   (yes:true, no:false, default:true) */
#define WP	(SD_WE)		/* Write protected (yes:true, no:false, default:false) */

#define	FCLK_SLOW()	{ unsigned long fpb = GetSystemClock() / (1 << mOSCGetPBDIV()); SpiChnSetBrg(2, SPICalutateBRG(fpb, 400000)); delay_us(5000); }

#define	FCLK_FAST()	{ unsigned long fpb = GetSystemClock() / (1 << mOSCGetPBDIV()); SpiChnSetBrg(2, SPICalutateBRG(fpb, 2000000)); delay_us(5000); }
...
void power_on (void)
{
	OpenSPI2(FATFS_SPI_START_CFG_1_1, FATFS_SPI_START_CFG_2);
}
...
BYTE xchg_spi (BYTE dat)
{
	...
	putcSPI2((unsigned int) dat);
	rx_data = (BYTE)getcSPI2();
...
#define RCVR_SPI_MULTI(dst,cnt)	{UINT c=cnt/2; BYTE *p=dst; do {putcSPI2(0xff); *p++=(BYTE)getcSPI2(); putcSPI2(0xff); *p++=(BYTE)getcSPI2(); } while(--c);}
...
static unsigned char SPICalutateBRG(unsigned int pb_clk, unsigned int spi_clk)
{
    unsigned int brg;
...
{
	BYTE i, n, cmd, ty, ocr[4];

	disk_timerproc(); // update Stat variable
...
//	UINT n = Timer1;					/* 1000Hz decrement timer with zero stopped */
//	if (n) Timer1 = --n;
//	n = Timer2;
//	if (n) Timer2 = --n;
